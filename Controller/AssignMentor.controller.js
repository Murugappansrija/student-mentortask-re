...
assignMentor.post("/mentor", async function (req, res) {
  try {
    const { name,emailid,skills } = req.body;
    let mentor;
    mentor = new mentorModel({ name ,emailid,skills});
...
assignMentor.post("/student", async function (req, res) {
  try {
    const { name,emailid, language, program } = req.body;
    let student;
    student = new studentModel({ name,emailid, language, program});
...
assignMentor.post(
  ...
  async function (req, res) {
    try {
      ...
      if (!student || !mentor) {
        res.status(404).json({ message: "Not found a data" });
      }
...
//Assign or change mentor for particular students
assignMentor.put(
...
assignMentor.get("mentor/:mentorId/students", async function (req, res) {
  try {
    ...
    res.status(200).json({data:mentor.assignedStudent})
  } catch (error) {
    console.log(error);
  }
});
